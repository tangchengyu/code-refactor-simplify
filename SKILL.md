...
description: >
  ...
  合并重复模块、修复语法错误（如智能引号）、消除死代码参数、将派生状态替换为直接计算、
  维护前后端分离和模块边界清晰时，
  务必触发此技能。适用于用户说"简化代码"、"重构"、"优化结构"、"清理代码"、"整理"、
  "改善代码质量"、"消除重复"、"前后端分离"、"分层"等场景。此技能处理的是重构/简化
  （reuse、simplification、efficiency、altitude）和职责边界审查，而非正确性检查（bug 修复由 /code-review 处理）。
---
...
- 检查前后端分离和职责边界是否清晰：
  - 前端/UI 层不直接承载持久化、权限、核心业务规则或后端资源访问逻辑
  - 后端/主进程/API 层不混入展示状态、DOM 操作、组件布局或交互细节
  - 跨层通信应通过明确接口、IPC/API、DTO 或适配层完成，而不是互相读取内部状态
  - 若单体桌面应用没有真实后端，也要保持 renderer、preload、main、service/store 等边界清楚
